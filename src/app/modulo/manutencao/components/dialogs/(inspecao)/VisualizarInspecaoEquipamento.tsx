...
export function VisualizarInspecaoDialog({ dados }: DadosInspecaoProps) {
  ...
  const dataHoraFim = dados.finalizadoEm ? new Date(dados.finalizadoEm) : new Date()

  const duracaoInspecaoHoras = differenceInHours(dataHoraFim, dataHoraInicio)
  const duracaoInspecaoMinutos = differenceInMinutes(dataHoraFim, dataHoraInicio)
          ...
          <>
            <div className="flex flex-row justify-between space-x-2 select-none">
              ...
              <div className="grid p-4 border rounded w-full">
                ...
                <span className={cn("capitalize text-xl", dados.statusInspecao === 'reprovado' ? 'text-red-700' : 'text-green-700')}>{dados.statusInspecao}</span>
              </div>
              <div className="grid p-4 border rounded w-full">
                <span className="text-start text-md">Duração: </span>
                <span className="text-xl">{duracaoInspecaoHoras > 0 ? `${duracaoInspecaoHoras} hs` : `${duracaoInspecaoMinutos} min`}</span>
              </div>
            </div>
            <ScrollArea className="max-h-72 w-full">
              {listaPontosInspecao?.map((ponto) => (
                <div key={ponto.pecasEquipamento.id} className="flex-1 flex-row justify-between my-2 p-2 rounded-md border w-full select-none">
                  <div className="flex justify-between flex-row-reverse mb-4 mr-2 items-center">
                    <span className="text-gray-200 text-xs">{`Inspecionado em: ${formatarDataBrasil(ponto.inspecionadoEm ? new Date(ponto.inspecionadoEm) : new Date())}`}</span>
                    <span className="text-start text-xl">{`Ponto: ${ponto.pecasEquipamento.nome}`}</span>
                  </div>
                  <span className={cn("text-start text-xl", ponto.aprovado ? 'text-green-700' : 'text-red-700')}>{`${ponto.aprovado ? 'Conforme' : 'Não Conforme'}`}</span>
                  <p className="text-justify">{ponto.pecasEquipamento.descricao}</p>
                </div>
              ))}
            </ScrollArea>
          </>
