...
export const colunasInspecoesEquipamento: ColumnDef<DadosInspecoesEquipamentoType>[] =
  [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="w-auto">
          {formatarDataBrasil(new Date(row.getValue('iniciadoEm')))}
        </div>
      ),
    },
    {
      ...
      enableHiding: false,
      cell: ({ row }) => {
        const usuario: string = row.original.usuario.pessoa.nome
        return <div className="capitalize md:w-full">{usuario}</div>
      },
    },
    {
    ...
    },
    {
      ...
      enableHiding: false,
      cell: ({ row }) => (
        <div className="capitalize md:w-full">
          {row.getValue('finalizadoEm')
            ? formatarDataBrasil(new Date(row.getValue('finalizadoEm')))
            : 'Em andamento'}
        </div>
      ),
    },
