...
import { z } from "zod";
import { atualizarPecaEquipamento } from "../../api/EquipamentoAPi";
...
export function FormularioEdicaoPeca({ idPeca, idEquipamento, nome, descricao }: EdicaoPecaProps) {
  ...
  const { mutateAsync: atualizarPeca } = useMutation({
    ...
    onError: (error) => {
      toast.error('Erro ao atualizar peça do equipamento', {
    ...
    onSuccess: (data) => {
      ...
      toast.success('Peça do equipamento atualizado com sucesso!')
